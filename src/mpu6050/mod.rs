...
pub(crate) fn setup<T: twim::Instance>(twim: &mut twim::Twim<T>, address: u8) {
    ...
    write_firmware(twim, address, true);
...
pub(crate) fn write_firmware<T: twim::Instance>(twim: &mut twim::Twim<T>, address: u8, verify: bool) {
...
pub(crate) fn reset_fifo<T: twim::Instance>(twim: &mut twim::Twim<T>, address: u8) {
    let mut reg_buffer = [0; 1];
...
pub(crate) async fn get_fifo_count<T: twim::Instance>(twim: &mut twim::Twim<'_, T>, address: u8) -> u16 {
...
#[allow(dead_code)]
pub(crate) fn test_panic<T: twim::Instance>(twim: &mut twim::Twim<T>, address: u8, basic_fifo: bool) {
    
    if basic_fifo {
        twim.blocking_write(address, &[35,  0b01111000]).unwrap(); //regular fifo on for accel/gyro
    }

    reset_fifo(twim, address);
    let mut count = 0;
...
pub(crate) async fn read_latest<T: twim::Instance>(twim: &mut twim::Twim<'_, T>, address: u8) -> [u8; dmp_firmware::DMP_PACKET_SIZE] {
    reset_fifo(twim, address);

    let mut count = get_fifo_count(twim, address).await;
    while count < 28 {
        count = get_fifo_count(twim, address).await;
    }
